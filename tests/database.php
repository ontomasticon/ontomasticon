...
dbQuery("DELETE FROM `terms` WHERE `shortname` IN ('echo', 'reverb', 'silence');");
...
dbQuery("UPDATE `terms` SET `broader` = ? WHERE `shortname` = 'bird_song';", array(termRow("animal_sound")["id"]));
...
dbQuery("UPDATE `terms` SET `broader` = NULL WHERE `shortname` = 'bird_song';");
...
dbQuery("UPDATE `terms` SET `broader` = ? WHERE `shortname` = 'animal_sound';", array(termRow("bird_song")["id"]));
dbQuery("UPDATE `terms` SET `broader` = ? WHERE `shortname` = 'bird_song';", array(termRow("animal_sound")["id"]));
termForm(array("name" => "Sound", "broader" => "animal_sound"));
list($out, $ok) = capture(function() { return(editTerm()); });
check("stops following links at a loop saved before loops were refused", $ok && termRow("sound")["broader"] == termRow("animal_sound")["id"]);
dbQuery("UPDATE `terms` SET `broader` = NULL WHERE `shortname` IN ('sound', 'bird_song');");
dbQuery("UPDATE `terms` SET `broader` = ? WHERE `shortname` = 'animal_sound';", array(termRow("sound")["id"]));
...
dbQuery("DELETE FROM `terms` WHERE `shortname` = 'noise';");
list($out, $ok) = capture(function() { return(deleteTerm()); });
...
dbQuery("DELETE FROM `terms` WHERE `shortname` = 'redbreast';");
list($out, $ok) = capture(function() { return(deleteCV()); });
...
$db->query("ALTER TABLE `terms` MODIFY COLUMN `language` VARCHAR(5) DEFAULT NULL;");
$out = runUpdate();
